...
/**								  **/
/**   <Gabriel Fernades Marchesano>	<15453229>   <Turma 02>     **/
/**								  **/
/*********************************************************************/
...
int buscarPalavraAux(PONT atual, char* palavra, int pos, int n) {
    // Se chegamos ao final da palavra
    if (pos == n) { // Verifica se a posição atual é igual ao comprimento da palavra
        return atual->contador; // Retorna o contador do nó atual
    }

    // Calcula o índice para o caractere atual da palavra
    int index = palavra[pos] - 'a'; // Converte 'a' para 0, 'b' para 1, etc.

    // Verifica se o filho correspondente existe
    if (atual->filhos == NULL || atual->filhos[index] == NULL) {
        return 0; // Não existe a palavra na trie
    }

    // Chama recursivamente para o próximo caractere
    return buscarPalavraAux(atual->filhos[index], palavra, pos + 1, n);
}

int buscarPalavra(PONT raiz, char* palavra, int n) {

    int result;
    // Chama a função auxiliar a partir da raiz
    result = buscarPalavraAux(raiz, palavra, 0, n);
    return result;
...
void inserir(PONT raiz, char* palavra, int n){
    ...
    for (int pos = 0; palavra[pos] != '\0'; pos++) {
        int index = palavra[pos] - 'a'; // Converte 'a' para 0, 'b' para 1, etc.

        // Se o nó atual não possui filhos, cria o arranjo de filhos
        if (atual->filhos == NULL) {
            atual->filhos = (PONT*)malloc(26 * sizeof(PONT));
            for (int i = 0; i < 26; i++) {
                atual->filhos[i] = NULL; // Inicializa todos os filhos como NULL
            }
        }
    ...
    atual->contador++; // Incrementa o contador da palavra
...
int excluirAux1(PONT atual, char* palavra, int pos) {
    // Se chegamos ao final da palavra
    if (palavra[pos] == '\0') {
        // Se a palavra não existe na trie
        if (atual->contador == 0) {
            return false;
        }
        // Marca a palavra como excluída
        atual->contador = 0;
        // Se não tem filhos, pode ser removido
        return (atual->filhos == NULL);
    }

    int index = palavra[pos] - 'a';
    
    // Se não existe o caminho para a palavra
    if (atual->filhos == NULL || atual->filhos[index] == NULL) {
        return false;
    }

    // Tenta excluir recursivamente
    if (excluirAux1(atual->filhos[index], palavra, pos + 1)) {
        PONT filho = atual->filhos[index];
        free(filho);  // Libera o nó filho
        ...
        // Se não há outros filhos
        if (!temOutrosFilhos) {
            free(atual->filhos);  // Libera o arranjo de filhos
            atual->filhos = NULL;
            // O nó atual pode ser removido se não for final de outra palavra
            return (atual->contador == 0);
        }
    }
    
    return false;  // Mantém o nó atual
}

void excluirTodas(PONT raiz, char* palavra, int n) {
    if (!raiz || !palavra || n <= 0) return;
    excluirAux1(raiz, palavra, 0);
}
...
*/
int excluirAux2(PONT atual, char* palavra, int pos) {
    if (palavra[pos] == '\0') {
        if (atual->contador == 0) {
            return false;
        }
        atual->contador--;
        // Se o contador chegou a zero e não tem filhos, marca para remoção
        return (atual->contador == 0 && atual->filhos == NULL);
    }
    ...
    if (excluirAux2(atual->filhos[index], palavra, pos + 1)) {
        PONT filho = atual->filhos[index];
        free(filho);
        atual->filhos[index] = NULL;
...
void excluir(PONT raiz, char* palavra, int n) {
    ...
    // Se for a última cópia da palavra, usa excluirTodas
    if (buscarPalavra(raiz, palavra, n) == 1) {
        excluirTodas(raiz, palavra, n);
    } else {
        excluirAux2(raiz, palavra, 0);
    }
}
